  ...
  return (
    <div className="space-y-8 fade-in" onClick={() => setOpenDropdown(null)}>
      ...
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">Gestiona las relaciones entre coaches y clientes</p>
      </div>
                  ...
                  <div className="p-4 border-b border-gray-200 bg-gray-50">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{assignedClients.length} clientes asignados</p>
                      </div>
                    </div>
                  </div>
                  <div className="p-2 flex-1 max-h-48 overflow-y-auto">
                    {assignedClients.length === 0 ? (
                      <p className="text-center text-xs text-gray-500 py-4">Sin clientes asignados</p>
                    ) : (
                      <div className="space-y-1">
                        {assignedClients.map(client => (
                          <div key={client.id} className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-100 group transition-colors">
                            ...
                            <button
                              ...
                              className="text-xs text-gray-500 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-all"
                            >
        ...
        <div className="space-y-4">
          ...
          <div className="card p-4 space-y-3">
            {unassignedClients.length === 0 ? (
              <p className="text-center text-sm text-gray-500 py-6">Todos los clientes tienen coach</p>
            ) : (
              unassignedClients.map(client => (
                <div key={client.id} className="p-3 rounded-xl bg-gray-50 border border-gray-200">
                  <p className="font-medium text-gray-900 text-sm mb-1">{client.name}</p>
                  ...
                  <div className="relative">
                    <button
                      ...
                      className="w-full flex items-center justify-between px-3 py-2 text-xs bg-white border border-gray-300 rounded-lg text-gray-800 hover:border-gray-300 transition-colors"
                    >
                    ...
                    {openDropdown === client.id && (
                      <div className="absolute top-full left-0 right-0 mt-1 bg-gray-100 border border-gray-300 rounded-lg shadow-xl overflow-hidden z-10">
                        {coaches.map(coach => (
                          <button
                            key={coach.id}
                            onClick={() => handleAssign(client.id, coach.id)}
                            className="w-full text-left px-3 py-2 text-xs text-gray-800 hover:bg-blue-100 hover:text-blue-500 transition-colors"
                          >
                            {coach.name}
                          </button>
                        ))}
