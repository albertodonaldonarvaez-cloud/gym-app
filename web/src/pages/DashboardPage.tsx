        ...
        <>
          {adminStats && (
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
              <StatCard icon={Users}      label="Total Usuarios"   value={adminStats.totalUsers}    sub="registrados"          color="bg-blue-50 text-blue-600" />
              <StatCard icon={Shield}     label="Coaches"          value={adminStats.totalCoaches}  sub="entrenadores"         color="bg-violet-50 text-violet-600" />
              <StatCard icon={UserCheck}  label="Clientes"         value={adminStats.totalClients}  sub="atletas activos"      color="bg-emerald-50 text-emerald-600" />
              <StatCard icon={Dumbbell}   label="Ejercicios"       value={adminStats.totalExercises} sub="catálogo disponible" color="bg-amber-50 text-amber-600" />
              <StatCard icon={UserX}      label="Sin Asignar"      value={adminStats.unassigned}    sub="necesitan coach"      color="bg-red-50 text-red-600" />
            </div>
          )}
          ...
          <div className="card p-0 overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              ...
              <Link to="/users" className="text-blue-600 text-xs hover:text-blue-700 transition-colors">
                Ver todos →
              </Link>
            </div>
                    ...
                    return (
                      <tr key={u.id}>
                        ...
                        <td className="py-4 px-6 text-sm text-gray-500">
                          {u.coach ? u.coach.name : u.role === 'CLIENT' ? <span className="text-red-500 text-xs">Sin asignar</span> : '—'}
                        </td>
                        <td className="py-4 px-6 text-sm text-gray-500">
      ...
      {user?.role === 'COACH' && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <StatCard icon={Users}      label="Mis Atletas"      value={athletes.length}   sub="asignados"          color="bg-blue-50 text-blue-600" />
            <StatCard icon={Dumbbell}   label="Ejercicios DB"    value={totalExercises}    sub="catálogo total"     color="bg-violet-50 text-violet-600" />
            <StatCard icon={Activity}   label="Rutinas Activas"  value={athletes.length}   sub="en seguimiento"     color="bg-emerald-50 text-emerald-600" />
          </div>
          ...
          <div className="card p-0 overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold text-gray-900">Tus Atletas</h2>
              </div>
            </div>
            <div className="overflow-x-auto">
              <table className="table-base">
