...
function ExercisePicker({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm"
         onClick={onClose}>
      <div className="w-full max-w-3xl bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl flex flex-col max-h-[85vh]"
           onClick={e => e.stopPropagation()}>
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800">
          <div className="flex items-center gap-2">
            <Dumbbell className="w-5 h-5 text-brand-400" />
            <h3 className="font-semibold text-white">Catálogo de Ejercicios</h3>
        ...
        <div className="flex flex-1 overflow-hidden">
            ...
            {loading ? (
              ...
              <ul>
                {exercises.map(ex => (
                  <li key={ex.id}
                      className={clsx(
                        'flex items-center gap-3 px-4 py-3 cursor-pointer border-b border-slate-800/40',
                        'hover:bg-slate-800/40 transition-colors',
                        hoveredEx?.id === ex.id && 'bg-slate-800/40'
                      )}
          ...
          {/* Preview panel */}
          {hoveredEx && (
            <div className="w-56 border-l border-slate-800 p-4 flex flex-col gap-3 slide-in overflow-hidden">
              ...
              <div>
                <p className="font-semibold text-white text-sm leading-tight">{hoveredEx.name}</p>
                <p className="text-xs text-slate-500 mt-1">{hoveredEx.category} · {hoveredEx.equipment}</p>
              </div>
...
function RoutineExerciseRow({
  ...
  return (
    <div className={clsx(
      'border border-slate-800 rounded-xl overflow-hidden transition-all',
      'hover:border-slate-700 bg-slate-900/50'
    )}>
      {/* Main row */}
      <div className="flex items-center gap-3 px-4 py-3">
        <GripVertical className="w-4 h-4 text-slate-700 cursor-grab shrink-0" />
        <span className="w-5 h-5 rounded-md bg-brand-500/15 text-brand-400 text-xs font-bold flex items-center justify-center shrink-0">
          {index + 1}
        </span>
        ...
        <div className="w-8 h-8 rounded-lg overflow-hidden bg-slate-800 shrink-0">
          {imgUrl ? (
            <img src={imgUrl} alt="" className="w-full h-full object-cover"
                 onError={e => { e.currentTarget.style.display='none' }} />
          ) : <Dumbbell className="w-4 h-4 text-slate-600 m-auto mt-1.5" />}
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <div className="flex items-center gap-1">
            ...
            <input type="number" min={1} max={20}
                   className="w-10 bg-slate-800 border border-slate-700 rounded-lg px-1.5 py-1 text-xs text-center text-slate-200 focus:outline-none focus:border-brand-500"
                   value={item.sets}
        ...
        <button onClick={onRemove} className="btn-ghost p-1.5 text-red-500 hover:text-red-400 hover:bg-red-500/10">
      ...
      {/* Expanded params */}
      {expanded && (
        <div className="px-4 pb-4 pt-1 border-t border-slate-800/60 grid grid-cols-3 gap-3 fade-in">
          <div>
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white">Creador de Rutinas</h1>
          <p className="text-slate-500 text-sm mt-1">Diseña y asigna planes de entrenamiento</p>
        </div>
        {selectedAthlete && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="card">
          ...
          {selectedAthleteData && (
            <div className="mt-3 flex items-center gap-2 p-2.5 rounded-lg bg-brand-500/8 border border-brand-500/15">
              ...
              <div>
                <p className="text-xs font-semibold text-slate-300">{selectedAthleteData.name}</p>
                <p className="text-[10px] text-slate-500">{selectedAthleteData.goal}</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          {DAYS.map(day => {
            ...
            return (
              ...
              >
                <span>{day}</span>
                {count > 0 && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-brand-500/20 text-brand-400 font-semibold">
                    {count}
                  </span>
                )}
              </button>
        ...
        <div className="lg:col-span-3 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-semibold text-slate-200">{selectedDay}</h2>
              {currentDay.focus && <p className="text-xs text-slate-500 mt-0.5">🎯 {currentDay.focus}</p>}
            </div>
            <button
              id="btn-add-exercise"
          ...
          {!selectedAthlete ? (
            <div className="card flex flex-col items-center justify-center py-16 gap-3 text-slate-600">
            ...
            <div className="card border-dashed border-slate-700 flex flex-col items-center justify-center py-16 gap-3 text-slate-600 hover:border-brand-500/40 hover:text-slate-500 transition-colors cursor-pointer"
                 onClick={() => setShowPicker(true)}>
